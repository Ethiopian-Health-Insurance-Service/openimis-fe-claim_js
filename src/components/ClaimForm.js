...
class ClaimForm extends Component {
  ...
  _save = (claim) => {
    let uncoveredServicesOrItems = [];
    if (!!this.state.claim.services) {
      let services = [...this.state.claim.services];
      services.forEach((item) => {
        if (item?.service?.covered == false) {
          uncoveredServicesOrItems.push(item.service.name);
        }
      })
    }
    if (!!this.state.claim.items) {
      ...
      items.forEach((item) => {
        if (item?.item?.covered == false) {
          uncoveredServicesOrItems.push(item.item.name);
        }
      })
    }

    if (uncoveredServicesOrItems.length > 0) {

      let confirm = () =>
        this.props.coreConfirm(
          formatMessage(this.props.intl, "claim", "uncoveredServices.title"),
          formatMessageWithValues(this.props.intl, "claim", "uncoveredServices.message",
            {
              servicesItems: `${uncoveredServicesOrItems.map((e) => e).join(",")}`,
            }),
        );
      let confirmedAction = () => {
  ...
  render() {
    ...
    const { claim, claim_uuid, lockNew, isSaved, saving } = this.state;
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators(
    { fetchClaim, claimHealthFacilitySet, journalize, print, generate, fetchMutation, coreConfirm },
    dispatch,
  );
};
